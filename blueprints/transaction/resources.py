...
class TransactionResource(Resource):
    ...
    def post(self):       
        ...
        if cart_qry == []:
            return {'status': 'FAILED', 'note': 'ZERO_CART'}, 404
    ...
    def put(self, transaction_id):
        ...
        if qry is None:
            return {'status': 'Transaction Details Not Found'}, 404, {'Content-Type': 'application/json'}
...
class TransactionList(Resource):
    ...
    def get(self):
        ...
        if args['orderby'] is not None:
            if args['orderby'] == 'id':
                if args['sort'] == 'desc':
                    qry = qry.order_by(desc(Transaction.transaction_id)) # bisa gini
                else:
