...
class CartResource(Resource):
    ...
    def post(self):
        ...
        for item in qry:
            if item.product_id == int(data['product_id']):
                ...
                if product_stock < (item.qty + int(data['qty'])):
                    return {'status': 'Product ID: '+ str(item.product_id) +' Stock Not Enough'}, 404, {'Content-Type': 'application/json'}
        ...
        if product_stock < int(data['qty']):
            return {'status': 'Stock Not Enough'}, 404, {'Content-Type': 'application/json'}
        else:
    ...
    def delete(self, cart_id):
        ...
        if qry is None:
            return {'status':'CART_NOT_FOUND'}, 404, {'Content-Type': 'application/json'}
        
        # to check if the product is this user's
        if qry.user_id != int(claims['user_id']):
            return {'status':'INVALID_ACTION'}, 404, {'Content-Type': 'application/json'}
